...
export const ComponentView = memo(({ component }: { component: ComponentConfig }) => {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-10 flex flex-col gap-10">

        {/* ── BREADCRUMB ── */}
        <nav className="flex items-center gap-1 text-[11.5px] text-zinc-400 dark:text-zinc-600">
          <span className="hover:text-zinc-600 dark:hover:text-zinc-400 cursor-pointer transition-colors">Components</span>
          <ChevronRight size={11} className="text-zinc-300 dark:text-zinc-700" />
          <span className="hover:text-zinc-600 dark:hover:text-zinc-400 cursor-pointer transition-colors">{meta.label}</span>
          <ChevronRight size={11} className="text-zinc-300 dark:text-zinc-700" />
          <span className="text-zinc-600 dark:text-zinc-400 font-medium">{component.name}</span>
        </nav>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex items-start justify-between gap-4">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2.5">
                <span className={`inline-flex items-center gap-1.5 text-[10.5px] font-semibold px-2 py-0.5 rounded-full ${meta.bg} ${meta.text}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${meta.dot}`} />
                  {meta.label}
                </span>
                <span className="text-[11px] text-zinc-300 dark:text-zinc-700">·</span>
                <span className="text-[11px] text-zinc-400 dark:text-zinc-600 font-medium tracking-wide">Nexora UI</span>
              </div>

              <h1 className="text-[30px] font-semibold tracking-tight leading-[1.1] text-zinc-900 dark:text-zinc-50">
                {component.name}
              </h1>

              <p className="text-[13.5px] text-zinc-500 dark:text-zinc-400 leading-relaxed max-w-[440px]">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {TAGS.map((tag) => (
              <span
                key={tag}
                className="text-[11px] font-medium px-2 py-0.5 rounded-md bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-500"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex flex-col rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          <div className="flex items-center gap-2.5 px-4 py-2.5 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-950/60">
              ...
              {STEPS.map((s, idx) => (
                <div key={s.n} className="flex gap-3">
                  <div className="flex flex-col items-center shrink-0">
                    <div className="w-5 h-5 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-[10px] font-bold text-zinc-500 dark:text-zinc-400 shrink-0">
                      {s.n}
                    </div>
                    {idx < STEPS.length - 1 && (
                      <div className="w-px flex-1 my-1 bg-zinc-100 dark:bg-zinc-800 min-h-[24px]" />
                    )}
                  </div>
                  <div className={`flex flex-col gap-0.5 ${idx < STEPS.length - 1 ? "pb-5" : ""}`}>
        ...
        {/* ── PREVIEW / CODE ── */}
        <div className="flex flex-col rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-950/60">
            ...
            <span className="text-[10.5px] text-zinc-400 dark:text-zinc-600 font-medium font-mono">
              {tab === "code"
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <a
            href="#"
            className="text-[11.5px] text-zinc-400 dark:text-zinc-600 hover:text-red-500 dark:hover:text-red-400 transition-colors duration-150 font-medium"
          >
            Report an issue →
          </a>
        </div>
